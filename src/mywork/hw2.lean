...
- For any rule that's not valid, have Z3 return a counterexample (the case in which the inference is false), 
and translate the formal counter-example into a concrete example (model of negation of rule is a counter example)
