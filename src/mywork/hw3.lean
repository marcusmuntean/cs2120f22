...
ANSWER HERE: Suppose Plato is an instance of a variable Person. We also
have a predicate, isMortal, which takes any Person as an argument and
outputs a proposition. Then, we have a proof called everyoneIsMortal 
which says that every Person is mortal. Lastly, we take this generalized
proof of everyoneIsMortal and apply it to the particular instance Plato 
to gain the proof that Plato is mortal.
-/
